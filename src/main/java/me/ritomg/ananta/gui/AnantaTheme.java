...
/*still in work based off ImpactTheme
*Yak code - RitomG
* Yak settings
*/
public class AnantaTheme extends ThemeBase {
    int height,padding;

    public AnantaTheme(IColorScheme scheme, int height, int padding) {
        super(scheme);
        this.height=height;
        this.padding=padding;
        scheme.createSetting(this,"Title Color","The color for panel titles.",true,true,new Color(16,16,16,198),false);
        scheme.createSetting(this,"Background Color","The panel background color.",true,true,new Color(174, 176, 178, 255),false);
        scheme.createSetting(this,"Text Background Color","The panel background color.",true,true,new Color(174, 176, 178, 255),false);
        ...
        scheme.createSetting(this,"Border Color","The colors to render resizable borders.",true,true,new Color(0,255,0),false);
        scheme.createSetting(this,"Overlay Color","The colors to render overlays.",false,true,new Color(0,0,200),false);
        ...
        scheme.createSetting(this,"Cursor Color", "The color for Cursor", false,true,new Color(255,255,255),false);
        ...
        scheme.createSetting(this,"Button Color","The main color for button text.",false,true,new Color(255,255,255),false);
        scheme.createSetting(this,"Tooltip Color","The color for description tooltips.",true,true,new Color(0,0,0,128),false);
    }

    protected void renderBackground (Context context, boolean focus, String type) {
        Color color = getBackgroundColor(focus);
        if (type.equalsIgnoreCase("")) color = getBackgroundColor(focus);
        if (type.equalsIgnoreCase("Text")) color = scheme.getColor("Text Background Color");
    ...
    protected void renderOverlay (Context context) {
        if (context.isHovered()) {
            Color color=new Color(12, 31, 203,24);
    ...
    protected void renderSmallButton(Context context, String title, int symbol, boolean focus) {
        ...
        Color color=scheme.getColor("Button Color");
        if (context.isHovered()) color=scheme.getColor("Hovered Font Color");
        switch (symbol) {
            case ITheme.CLOSE:
        ...
        if (title!=null) context.getInterface().drawString(new Point(context.getPos().x+(symbol==ITheme.NONE?padding:context.getSize().height),context.getPos().y+padding),height,title,scheme.getColor("Button Color"));
    }
    ...
    public IDescriptionRenderer getDescriptionRenderer() {
        return (inter, pos, text) -> {
            ...
            inter.drawString(new Point(pos.x+padding-1,pos.y+padding-1),height,text,getFontColor(false));
        };
    }
    ...
    public IContainerRenderer getContainerRenderer(int logicalLevel, int graphicalLevel, boolean horizontal) {
        return new IContainerRenderer() {
            ...
            public void renderBackground (Context context, boolean focus) {
                if (graphicalLevel==0) AnantaTheme.this.renderBackground(context,focus, "");
    ...
    public <T> IPanelRenderer<T> getPanelRenderer(Class<T> type, int logicalLevel, int graphicalLevel) {
        return new IPanelRenderer<T>() {
            ...
            public void renderPanelOverlay(Context context, boolean focus, T state, boolean open) {
                Color color=scheme.getColor("Overlay Color");
            ...
            public void renderTitleOverlay(Context context, boolean focus, T state, boolean open) {
                if (graphicalLevel<=0) {
                    Color colorA=scheme.getColor("Overlay Color");
    ...
    public <T> IScrollBarRenderer<T> getScrollBarRenderer(Class<T> type, int logicalLevel, int graphicalLevel) {
        return new IScrollBarRenderer<T>() {
            ...
            public int renderScrollBar (Context context, boolean focus, T state, boolean horizontal, int height, int position) {
                renderBackground(context,focus, "");
    ...
    public <T> IEmptySpaceRenderer<T> getEmptySpaceRenderer(Class<T> type, int logicalLevel, int graphicalLevel, boolean container) {
        return (context, focus, state) -> {
            if (graphicalLevel==0) renderBackground(context,focus, "Empty");
        };
    }
            ...
            public void renderButton(Context context, String title, boolean focus, T state) {
                ...
                if (graphicalLevel<=0) {
                    if (container) {
                            ...
                            context.getInterface().fillRect(context.getRect(),color,color,color,color);
                    } else renderBackground(context,effFocus, "Button");
                }
                if (!container) {
                ...
                Color valueColor=scheme.getColor("Button Color");
                ...
                if (type==String.class) {
                    ...
                    if (context.isHovered() && container) {
                        ...
                        Color checkColor=scheme.getColor("Button Color");
            ...
            public void renderButton(Context context, String title, boolean focus, Void state) {
                if (graphicalLevel<=0) {
                    if (container) {
                        Color color=scheme.getColor("Title Color");
                        context.getInterface().fillRect(context.getRect(),color,color,color,color);
                    } else renderBackground(context,focus, "Button");
                }
                if (!container) {
                    Color color=graphicalLevel<=0?scheme.getColor("Panel Outline Color"):scheme.getColor("Component Outline Color");
                    ITheme.drawRect(context.getInterface(),context.getRect(),color);
            ...
            public void renderButton(Context context, String title, boolean focus, String state) {
                ...
                if (graphicalLevel<=0) {
                    if (container) {
                        Color color=scheme.getColor("Title Color");
                        context.getInterface().fillRect(context.getRect(),color,color,color,color);
                    } else renderBackground(context,effFocus, "Keybind");
                }
                if (!container) {
                    Color color=graphicalLevel<=0?scheme.getColor("Panel Outline Color"):scheme.getColor("Component Outline Color");
                    ITheme.drawRect(context.getInterface(),context.getRect(),color);
            ...
            public void renderSlider(Context context, String title, String state, boolean focus, double value) {
                ...
                if (graphicalLevel<=0) {
                    if (container) {
                        Color color=scheme.getColor("Title Color");
                        context.getInterface().fillRect(context.getRect(),color,color,color,color);
                    } else renderBackground(context,effFocus, "Slider");
                }
                if (!container) {
                    Color color=graphicalLevel<=0?scheme.getColor("Panel Outline Color"):scheme.getColor("Component Outline Color");
                    ITheme.drawRect(context.getInterface(),context.getRect(),color);
    ...
    public IResizeBorderRenderer getResizeRenderer() {
        return new IResizeBorderRenderer() {
            ...
            public void drawBorder(Context context, boolean focus) {
                Color color=scheme.getColor("Border Color");
                Rectangle rect=context.getRect();
                context.getInterface().fillRect(new Rectangle(rect.x,rect.y,rect.width +2,getBorder()),color,color,color,color);
                context.getInterface().fillRect(new Rectangle(rect.x,rect.y+rect.height-getBorder(),rect.width + 2,getBorder()),color,color,color,color);
                context.getInterface().fillRect(new Rectangle(rect.x,rect.y+getBorder(),getBorder() + 2,rect.height-2*getBorder()),color,color,color,color);
                context.getInterface().fillRect(new Rectangle(rect.x+rect.width-getBorder(),rect.y+getBorder(),getBorder() + 2,rect.height-2*getBorder()),color,color,color,color);
            }
    ...
    public ITextFieldRenderer getTextRenderer (boolean embed, int logicalLevel, int graphicalLevel, boolean container) {
        return new ITextFieldRenderer() {
            ...
            public int renderTextField (Context context, String title, boolean focus, String content, int position, int select, int boxPosition, boolean insertMode) {
                boolean effFocus=container?context.hasFocus():focus;
                renderBackground(context,effFocus, "Text");
                ...
                renderOverlay(context);
                context.getInterface().drawString(new Point(context.getPos().x+padding,context.getPos().y+padding/2),height,title+":",textColor);
                // Draw the box
            ...
            public Rectangle getTextArea (Context context, String title) {
                Rectangle rect=context.getRect();
                int length=padding+context.getInterface().getFontWidth(height,title+":");
    ...
    public IColorPickerRenderer getColorPickerRenderer() {
        return new StandardColorPicker() {
            ...
            public int getBaseHeight() {
                return AnantaTheme.this.getBaseHeight();
            }

            @Override
            public void renderCursor (Context context, Point p, Color color) {
                Color fontColor=scheme.getColor("Cursor Color");
                context.getInterface().fillRect(new Rectangle(p.x-1,p.y-1,4,4),fontColor,fontColor,fontColor,fontColor);
            }
        };
    }
    ...
    public Color getFontColor(boolean focus) {
        if (focus) return scheme.getColor("Hovered Font Color");
